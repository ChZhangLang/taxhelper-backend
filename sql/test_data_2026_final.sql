...
-- Create test users
INSERT INTO user (userAccount, userPassword, realName, idCard, taxRegion, userRole, isDelete)
SELECT '13800138001', 'e10adc3949ba59abbe56e057f20f883e', '虚伪陈', '110101199001010001', 'Beijing', 'user', 0
WHERE NOT EXISTS (SELECT 1 FROM user WHERE userAccount = '13800138001');

INSERT INTO user (userAccount, userPassword, realName, idCard, taxRegion, userRole, isDelete)
SELECT '13800138002', 'e10adc3949ba59abbe56e057f20f883e', '李浩宇', '310101199505050002', 'Shanghai', 'user', 0
WHERE NOT EXISTS (SELECT 1 FROM user WHERE userAccount = '13800138002');

INSERT INTO user (userAccount, userPassword, realName, idCard, taxRegion, userRole, isDelete)
SELECT '13800138003', 'e10adc3949ba59abbe56e057f20f883e', '丁真', '510101200012120003', 'Chengdu', 'user', 0
WHERE NOT EXISTS (SELECT 1 FROM user WHERE userAccount = '13800138003');

SET @user1 = (SELECT id FROM user WHERE userAccount = '13800138001');
SET @user2 = (SELECT id FROM user WHERE userAccount = '13800138002');
SET @user3 = (SELECT id FROM user WHERE userAccount = '13800138003');

-- User 1: High income (35000/month) - 3 special deductions
INSERT INTO tax_special_deduct (userId, deductType, status, isDelete) VALUES (@user1, 1, 1, 0), (@user1, 4, 1, 0), (@user1, 6, 1, 0);

INSERT INTO tax_record (userId, income, insurance, deduct, taxAmount, calcType, taxYear, taxMonth, cumulativeIncome, cumulativeInsurance, cumulativeDeduct, cumulativeTax, taxableIncome, beforeTaxIncome, isDelete) VALUES
...
INSERT INTO tax_record (userId, income, insurance, deduct, taxAmount, calcType, taxYear, cumulativeIncome, cumulativeInsurance, cumulativeDeduct, cumulativeTax, taxableIncome, beforeTaxIncome, isDelete) VALUES
(@user1, 420000.00, 63000.00, 48000.00, 44400.00, 2, 2026, 420000.00, 63000.00, 48000.00, 44400.00, 309000.00, 357000.00, 0);

-- User 2: Middle income (15000/month) - 1 special deduction (house rent)
INSERT INTO tax_special_deduct (userId, deductType, status, isDelete) VALUES (@user2, 5, 1, 0);
...
-- User 3: Low income (6000/month) - No special deduction, No tax
INSERT INTO tax_record (userId, income, insurance, deduct, taxAmount, calcType, taxYear, taxMonth, cumulativeIncome, cumulativeInsurance, cumulativeDeduct, cumulativeTax, taxableIncome, beforeTaxIncome, isDelete) VALUES
